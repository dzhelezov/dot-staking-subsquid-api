...
export class SumRewardResolver {
  ...
  @FieldResolver(() => StakingReward)
  async rewards(@Root() r: SumReward, @Ctx() ctx: BaseContext): Promise<StakingReward[] | null> {
    return ctx.dataLoader.loaders.SumReward.rewards.load(r);
  }

  @FieldResolver(() => StakingSlash)
  async slashs(@Root() r: SumReward, @Ctx() ctx: BaseContext): Promise<StakingSlash[] | null> {
    return ctx.dataLoader.loaders.SumReward.slashs.load(r);
  }
}
